...
public class APathfinding {
	...
	private Frame frame;
	private Cell startNode, endNode, par;
	private boolean diagonal, running, noPath, complete, trig;
	private ArrayList<Cell> borders, open, closed, path;
	...
	public APathfinding(Frame frame, int size, Cell start, Cell end) {
		this.frame = frame;
		this.size = size;
		startNode = start;
		endNode = end;
	...
	public void start(Cell s, Cell e) {
		running = true;
		startNode = s;
		startNode.setG(0);
		endNode = e;
	...
	public void setup(Cell s, Cell e) {
		running = true;
		startNode = s;
		startNode.setG(0);
		par = startNode;
	...
	public void findPath(Cell parent) {
		Cell openNode = null;
		...
		if (Cell.isEqual(parent, endNode)) {
		...
		if (diagonal) {
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					...
					int possibleX = (parent.getX() - size) + (size * i);
					int possibleY = (parent.getY() - size) + (size * j);
					Cell openCheck = getOpenNode(possibleX, possibleY);
	...
	public void calculateNodeValues(int possibleX, int possibleY, Cell openNode, Cell parent) {
		...
		// coordinates
		openNode = new Cell(possibleX, possibleY);
	...
	public void connectPath() {
		if (getPathList().size() == 0) {
			Cell parentNode = endNode.getParent();

			while (!Cell.isEqual(parentNode, startNode)) {
				addPath(parentNode);

				for (int i = 0; i < getClosedList().size(); i++) {
					Cell current = getClosedList().get(i);

					if (Cell.isEqual(current, parentNode)) {
	...
	public void addBorder(Cell node) {
		if (borders.size() == 0) {
			borders.add(node);
	...
	public void addClosed(Cell node) {
		if (closed.size() == 0) {
			closed.add(node);
	...
	public void removeOpen(Cell node) {
	...
	public boolean checkBorderDuplicate(Cell node) {
	...
	public boolean checkOpenDuplicate(Cell node) {
		for (int i = 0; i < open.size(); i++) {
			if (node.getX() == open.get(i).getX() && node.getY() == open.get(i).getY()) {
				return true;
			}
	...
	public boolean checkClosedDuplicate(Cell node) {
	...
	public ArrayList<Cell> getBorderList() {
		return borders;
	}
	...
	public Cell getOpen(int location) {
	...
	public ArrayList<Cell> getPathList() {
		return path;
	}
	...
	public Cell getOpenNode(int x, int y) {
		for (int i = 0; i < open.size(); i++) {
