...
public class Frame extends JPanel
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < pathfinding.getOpenList().size(); i++) {
			Cell current = pathfinding.getOpenList().get(i);
		...
		for (int i = 0; i < pathfinding.getClosedList().size(); i++) {
			Cell current = pathfinding.getClosedList().get(i);
		...
		for (int i = 0; i < pathfinding.getPathList().size(); i++) {
			Cell current = pathfinding.getPathList().get(i);
	...
	public void drawInfo(Cell current, Graphics g) {
		if (size > 50) {
	...
	public void MapCalculations(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			if (currentKey == 's') {
				...
				if (startNode == null) {
					startNode = new Cell(e.getX() - xRollover, e.getY() - yRollover);
				} else {
			...
			// If 'e' is pressed create end node
				...
				if (endNode == null) {
					endNode = new Cell(e.getX() - xRollover, e.getY() - yRollover);
				} else {
	...
	public void mouseWheelMoved(MouseWheelEvent m) {
		...
		for (int i = 0; i < pathfinding.getClosedList().size(); i++) {
			if (!Cell.isEqual(pathfinding.getClosedList().get(i), startNode)) {
